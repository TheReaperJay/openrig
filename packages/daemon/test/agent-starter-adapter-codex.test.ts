...
describe("Agent Starter v1 vertical — real Codex adapter delivery (M2 R2)", () => {
  it("real CodexRuntimeAdapter.deliverStartup writes STARTER content to AGENTS.md via guidance_merge", async () => {
    ...
    try {
      ...
      const realCodexLaunch = codexAdapter.launchHarness.bind(codexAdapter);
      (codexAdapter as { launchHarness: typeof codexAdapter.launchHarness }).launchHarness = async (binding, opts) => {
        const r = await realCodexLaunch(binding, opts);
        simulateSessionStart(agentActivityStore, { nodeId: binding.nodeId, runtime: "codex" });
        return r;
      };
